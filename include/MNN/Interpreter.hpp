...
//Shaquille, Added 20200902 Start
    int     GetSessionInputTensorIdx(const Session* session, const char* name) const;

    int     GetSessionOutputTensorIdx(const Session* session, const char* name) const;
//Shaquille, Added 20200902 End

//Shaquille, Added 20201108 Start
	Tensor* CreateSessionIOTensor(Session* session, Tensor* src_tensor, int tensor_io_type, int tensor_dim_type);
