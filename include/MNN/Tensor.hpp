...
//Shaquille, Added 20201107 Start
	/**
	* @brief create a tensor with same shape as given tensor.
	* @param ion_device_desc   device desc which own the backend's pointer
	* @param tensor            shape provider.
	* @param type              dimension type.
	* @warning tensor data won't be copied.
	*/
	Tensor(struct InsideDescribe*   ion_device_desc, 
		   const  Tensor*           tensor, 
...
//Shaquille, Added 20201107 Start
	/**
	* @brief create tensor with shape, data type, data and dimension type.
	* @param device_desc*   desc which own a device backend pointer,
	* @param shape          tensor shape.
	* @param type           data type.
	* @param dimType        dimension type.
	* @return created tensor.
	*/
	static Tensor* create_by_backend(struct InsideDescribe*   device_desc,
...
public:
     ...
     * @param deviceTensor  given device tensor, it include the backend.
     * @param copyData      copy data or not.
     * @return created host tensor.
	 * @note, user should release(delete) this tensor by theirselves before the release of the backend and the session!!!
	 *        the "used_by_user_" of deviceTensor will be set, if create successfully 
     */
	typedef enum {
		TENSOR_NORMAL = 0,
		TENSOR_SESSION_INPUT,
		TENSOR_SESSION_OUTPUT
	}TENSOR_IO_TYPE;

    static Tensor* createHostTensorFromDevice(const Tensor* deviceTensor, bool copyData, int tensor_io_type = TENSOR_IO_TYPE::TENSOR_NORMAL, int tensor_dim_type = -1);

	static bool    copy_to_host_batch(std::vector<Tensor*> const& dev_tensor, std::vector<Tensor*>& host_tensor);
...
//Shaquille, Added 20201107 Start
	inline void    SetUsedFlag(bool flag)      { used_by_user_ = flag ; } ;
	inline bool    IsUsedFlag()                { return used_by_user_ ; } ;
	inline void    SetIONFlag(bool flag)       { is_ion_buf_ = flag ; } ;
	inline bool    IsIONBuf() const            { return is_ion_buf_ ; } ;
	void           SyncIONStart(bool finish_cmd_que);
	void           SyncIONEnd(bool finish_cmd_que);
//Shaquille, Added 20201107 End
...
//Shaquille, Added 20201107 Start
	bool                   is_ion_buf_       = false;
	bool                   used_by_user_     = false;
//Shaquille, Added 20201107 End
private:
