...
int Interpreter::GetSessionInputTensorIdx(const Session* session, const char* name) const
{
    MNN_ASSERT(nullptr != session);
    int idx = session->getInputTensorIdx(name);
    return idx;
}

int Interpreter::GetSessionOutputTensorIdx(const Session* session, const char* name) const
{
    MNN_ASSERT(nullptr != session);
    int idx = session->getOutputTensorIdx(name);
    return idx;
}
//Shaquille, Added 20200902 End

//Shaquille, Added 20201108 Start
Tensor* Interpreter::CreateSessionIOTensor(Session* session, Tensor* src_tensor, int tensor_io_type, int tensor_dim_type)
{
	MNN_ASSERT(nullptr != session);
	Tensor*   tensor = session->CreateIOTensor(src_tensor, tensor_io_type, tensor_dim_type);
	return tensor;
}
