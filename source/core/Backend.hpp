...
//Shaquille, Added 20201107 Start
	virtual bool                           IsSupportION() const                                                                 { return false ; } ;
	virtual std::pair<uint8_t*, uint64_t>  CreateIONBuf(int buf_size)                                                           { return { nullptr, 0 }; };
	virtual void                           ReleaseIONBuf(void* host_buf)                                                        { ; } ;
	virtual void                           SyncIONBufStart(void* host_buf, bool finish_cmd_que)                                 { ; } ;
	virtual void                           SyncIONBufEnd(void* host_buf, bool finish_cmd_que)                                   { ; } ;
	virtual void                           onCopyIONBufferFromDev(const Tensor* srcTensor, const Tensor* dstTensor, bool block) { ; } ;
