...
//Shaquille, Added 20200902 Start
int Session::getInputTensorIdx(const char* name) const
{
    Tensor const* tensor = getInput(name);
    if(nullptr == tensor)
        return -1;
    
    int   all_tensor_count = (int)(mTensors.size());
    int   i                = 0;
    for(i = 0 ; i < all_tensor_count ; i ++)
    {
        Tensor const* cur_tensor = mTensors[i].second.get();
        if(tensor == cur_tensor)
            break;
    }
    if(i >= all_tensor_count)
        return -1;

    return i;
}

int Session::getOutputTensorIdx(const char* name) const
{
    Tensor const* tensor = getOutput(name);
    if(nullptr == tensor)
        return -1;
...
//Shaquille, Added 20200902 End

//Shaquille, Added 20201108 Start
Tensor* Session::CreateIOTensor(Tensor* src_tensor, int tensor_io_type, int tensor_dim_type)
{
	Tensor* tensor = MNN::Tensor::createHostTensorFromDevice(src_tensor, false, tensor_io_type, tensor_dim_type);
	if (MNN::Tensor::TENSOR_SESSION_OUTPUT == tensor_io_type)
	{
		if (nullptr != tensor)
		{
			Tensor::InsideDescribe* desc = TensorUtils::getDescribe(src_tensor);
			if(0 == desc->useCount)
				src_tensor->SetUsedFlag(true);
		}
	}
	return tensor;
}
