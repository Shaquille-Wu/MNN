...
//Shaquille, Added 20201215 Start
...
void MNNScaleAndAddBias(float* dst, const float* src, const float* bias, const float* alpha, size_t planeNumber,
                        size_t biasNumber) {
//Shaquille, Modified 20201210 Start          
...
#else
#ifdef MNN_USE_SSE
	MNNScaleAndAddBiasOpt(dst, src, bias, alpha, planeNumber, biasNumber);
#else
    for (int z = 0; z < biasNumber; ++z) {
        float* dstZ = dst + planeNumber * 4 * z;
        const float* srcZ = src + planeNumber * 4 * z;
...
#endif
#endif
//Shaquille, Modified 20201210 End
}
