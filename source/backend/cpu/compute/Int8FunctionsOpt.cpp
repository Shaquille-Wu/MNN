...
//Shaquille, Added 20201210 Start
#ifdef MNN_ORION_INT8_OPT
#include "CommonOptFunction.h"
#endif
//Shaquille, Added 20201210 End
...
//Shaquille, Added 20201210 Start
#ifdef MNN_ORION_INT8_OPT
void MNNScaleAddInt8(int8_t* dst, const int8_t* src0, const int8_t* src1, const float* scale0, const float* scale1, const float* outputScale, const size_t size)
{
#ifdef MNN_USE_SSE
    MNNScaleAddInt8Opt(dst, src0, src1, scale0, scale1, outputScale, size);
#else
    for (int i = 0; i < size; ++i)
    {
        for (int k = 0; k < 4; ++k) 
        {
            float sum   = static_cast<float>(src0[i * 4 + k]) * scale0[k] +
            static_cast<float>(src1[i * 4 + k]) * scale1[k];
            float value    = sum * outputScale[k];
            dst[i * 4 + k] = static_cast<int8_t>(std::max(std::min(value, 127.0f), -127.0f));
        }
    }
#endif
}
