...
//Shaquille, Added 20201208 Start
	void(*MNNScaleAndAddBias)(float* dst, const float* src, const float* bias, const float* alpha, size_t planeNumber, size_t biasNumber)=_SSE_MNNScaleAndAddBias;
    void(*MNNRoundFloat)(const float* src, float* dst, size_t size) = _SSE_MNNRoundFloat;
    void(*MNNSignFloat)(const float* src, float* dst, size_t size) = _SSE_MNNSignFloat;
    void(*MNNScaleAddInt8)(int8_t* dst, const int8_t* src0, const int8_t* src1, const float* scale0, const float* scale1, const float* outputScale, const size_t size) = _SSE_MNNScaleAddInt8;
...
//Shaquille, Added 20201208 Start
#ifdef MNN_ORION_INT8_OPT
void MNNScaleAndAddBiasOpt(float* dst, const float* src, const float* bias, const float* alpha, size_t planeNumber, size_t biasNumber)
{
    gFunc.MNNScaleAndAddBias(dst, src, bias, alpha, planeNumber, biasNumber);
}

void MNNScaleAddInt8Opt(int8_t* dst, const int8_t* src0, const int8_t* src1, const float* scale0, const float* scale1, const float* outputScale, const size_t size)
{
    gFunc.MNNScaleAddInt8(dst, src0, src1, scale0, scale1, outputScale, size);
}

void MNNRoundFloat(const float* src, float* dst, size_t size)
{
	gFunc.MNNRoundFloat(src, dst, size);
}

void MNNSignFloat(const float* src, float* dst, size_t size)
{
	gFunc.MNNSignFloat(src, dst, size);
}
#endif
//Shaquille, Added 20201208 End
