...
static void RoundFloat(const float* src, float* dst, size_t size, Backend* bn)
{
    auto backend = [bn]() {
        return bn;
    };
    ...
    auto   numberThread  = ((CPUBackend*)bn)->threadNumber();
    if (size >= 1024 && numberThread > 1)
    {
        size_t   block_size   = size / numberThread;
        elapsed_size          = block_size * numberThread;
        MNN_CONCURRENCY_BEGIN(tId, numberThread) {
            const float*  src_data = src + tId * block_size;
            float*        dst_data = dst + tId * block_size;
            MNNRoundFloat(src_data, dst_data, block_size);
        }
        MNN_CONCURRENCY_END();
    }
    if (elapsed_size < size)
        MNNRoundFloat(src + elapsed_size, dst + elapsed_size, size - elapsed_size);
}

static void SignFloat(const float* src, float* dst, size_t size, Backend* bn)
{
    auto backend = [bn]() {
        return bn;
    };
    ...
    if (size >= 1024 && numberThread > 1)
    {
        ...
        MNN_CONCURRENCY_BEGIN(tId, numberThread) {
            const float*  src_data = src + tId * block_size;
            float*        dst_data = dst + tId * block_size;
            MNNSignFloat(src_data, dst_data, block_size);
        }
        MNN_CONCURRENCY_END();
    }
    if (elapsed_size < size)
        MNNSignFloat(src + elapsed_size, dst + elapsed_size, size - elapsed_size);
}
#endif
...
//Shaquille, Modified 20201215 Start   
#if !defined(MNN_ORION_INT8_OPT) || !(defined(_M_IX86) || defined(_M_X64) || defined(__i386__) || defined(__x86_64__)) 
            return _unaryOp<UnarySign<float>, float>(input->host<void>(), output->host<void>(), input->elementSize(), backend());
#else
            SignFloat(input->host<float>(), output->host<float>(), input->elementSize(), backend());
            return NO_ERROR;
#endif
//Shaquille, Modified 20201215 End
        case UnaryOpOperation_ROUND:
//Shaquille, Modified 20201215 Start      
#if !defined(MNN_ORION_INT8_OPT) || !(defined(_M_IX86) || defined(_M_X64) || defined(__i386__) || defined(__x86_64__))      
            return _unaryOp<UnaryRound<float>, float>(input->host<void>(), output->host<void>(), input->elementSize(), backend());
#else
            RoundFloat(input->host<float>(), output->host<float>(), input->elementSize(), backend());
            return NO_ERROR;
#endif            
//Shaquille, Modified 20201215 End
