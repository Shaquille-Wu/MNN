...
//Shaquille, Modified 20201110 Start
#if 0
...
#else
	cl::Image&      o_img   = openCLImage(output);
	cl_image_format img_fmt = o_img.getImageInfo<CL_IMAGE_FORMAT>();
	cl::Event       event;
	cl_int          error   = 0;
	uint32_t outputGlobalWorkSize[2] = { static_cast<uint32_t>(UP_DIV(input->channel(), 4) * input->width()),
										 static_cast<uint32_t>(input->batch() * input->height()) };
	if (CL_FLOAT != img_fmt.image_channel_data_type || input->channel() > 4)
	{
		if (bufferToImageKernel.get() == nullptr) {
			std::set<std::string> buildOptions;
			buildOptions.emplace("-DBUFFER_IMAGE_IO_TRANS");
		...
		bufferToImageKernel.setArg(idx++, sizeof(outputImageShape), outputImageShape);
		bufferToImageKernel.setArg(idx++, UP_DIV(input->channel(), 4));
		bufferToImageKernel.setArg(idx++, o_img);
	...
	{
		error = runtime->commandQueue().enqueueCopyBufferToImage(openCLBuffer(input),
			                                                     o_img,
			                                                     0,
			                                                     { 0, 0, 0 },
			                                                     { outputGlobalWorkSize[0], outputGlobalWorkSize[1], 1 },
			                                                     nullptr,
			                                                     &event);
	}
	MNN_CHECK_CL_SUCCESS(error);
...
#endif
//Shaquille, Modified 20201110 End
    #ifdef ENABLE_OPENCL_TIME_PROFILER
        int costTime = (int)runtime->getCostTime(&event);
        MNN_PRINT("kernel cost:%d    us inputFormatTransform\n",costTime);
    #endif
