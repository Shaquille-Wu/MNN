...
//Shaquille, Modified 20201031 Start   
#if 0
	...
	int iH = 1;
	int iW = 1;
	if (3 == input->buffer().dimensions)
	{
		iN = 1;
		iC = (0 != input->buffer().dim[0].extent) ? input->buffer().dim[0].extent : 1;
		iH = (0 != input->buffer().dim[1].extent) ? input->buffer().dim[1].extent : 1;
		iW = (0 != input->buffer().dim[2].extent) ? input->buffer().dim[2].extent : 1;

		if (TensorUtils::getDescribe(input)->dimensionFormat == MNN::MNN_DATA_FORMAT_NHWC) {
			iN = 1;
			iH = (0 < input->buffer().dim[0].extent) ? input->buffer().dim[0].extent : 1;
			iW = (0 < input->buffer().dim[1].extent) ? input->buffer().dim[1].extent : 1;
			iC = (0 < input->buffer().dim[2].extent) ? input->buffer().dim[2].extent : 1;
		}
	}
	else
	{
	    iN = (0 != input->buffer().dim[0].extent) ? input->buffer().dim[0].extent : 1;
	    iC = (0 != input->buffer().dim[1].extent) ? input->buffer().dim[1].extent : 1;
	    iH = (0 != input->buffer().dim[2].extent) ? input->buffer().dim[2].extent : 1;
	    iW = (0 != input->buffer().dim[3].extent) ? input->buffer().dim[3].extent : 1;
...
#endif
//Shaquille, Modified 20201031 End
