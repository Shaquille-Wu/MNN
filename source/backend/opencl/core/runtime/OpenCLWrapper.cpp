...
//Shaquille, Added 20201225 Start
    "/system/lib64/libOpenCL_system.so",
//Shaquille, Added 20201225 End
    // Mali
...
#define MNN_LOAD_FUNCTION_PTR(func_name) func_name = reinterpret_cast<func_name##Func>(dlsym(handle_, #func_name)); \
    ...
    MNN_LOAD_FUNCTION_PTR(clCreateKernelsInProgram);
...
//Shaquille, Added 20201105 Start
    MNN_LOAD_FUNCTION_PTR(clEnqueueCopyBufferToImage);
    MNN_LOAD_FUNCTION_PTR(clEnqueueCopyImageToBuffer);
//Shaquille, Added 20201105 End
...
{
    auto func = MNN::OpenCLSymbolsOperator::getOpenclSymbolsPtr()->clCreateKernelsInProgram;
    MNN_CHECK_NOTNULL(func);
...
//Shaquille, Added 20201105 Start
cl_int CL_API_CALL clEnqueueCopyBufferToImage(cl_command_queue queue,
                                              cl_mem src_buffer,
                                              cl_mem dst_image,
                                              size_t src_offset,
                                              const size_t* dst_origin,
                                              ...
                                              cl_event* event)
{
    auto func = MNN::OpenCLSymbolsOperator::getOpenclSymbolsPtr()->clEnqueueCopyBufferToImage;
    MNN_CHECK_NOTNULL(func);
    return func(queue, src_buffer, dst_image, src_offset, dst_origin, region, num_events_in_wait_list, event_wait_list, event);
}

cl_int CL_API_CALL clEnqueueCopyImageToBuffer(cl_command_queue queue,
                                              cl_mem src_image,
                                              cl_mem dst_buffer,
                                              const size_t* src_origin,
                                              const size_t* region,
                                              ...
                                              cl_event* event)
{
    auto func = MNN::OpenCLSymbolsOperator::getOpenclSymbolsPtr()->clEnqueueCopyImageToBuffer;
    MNN_CHECK_NOTNULL(func);
    return func(queue, src_image, dst_buffer, src_origin, region, dst_offset, num_events_in_wait_list, event_wait_list, event);
}
//Shaquille, Added 20201105 End
