...
#ifdef LEAKY_RELU
                    res = select(((FLOAT)(LEAKY_RELU))*res, res, res >= (FLOAT4)0);
#endif
...
__kernel void winogradTransformDest(__read_only image2d_t uInput, // 0
    ...
    if (pos.x < unitWidth*unitHeight && pos.y < dstChannelC4) {
        ...
        int unitHeight_idx = pos.x / unitWidth;
        int2 realPos   = (int2)(unitWidth_idx + offsetX, unitHeight_idx + offsetY);
        int srcWidth   = (unitWidth * unitHeight + 3) >> 2;
        int dstXOrigin = unitWidth * unitHeight_idx + unitWidth_idx;
        int dstX       = dstXOrigin >> 2;
        int dstY       = (pos.y << 2) + (dstXOrigin & 3);
        int oz         = pos.y % dstChannelC4;
        FLOAT4 bias    = RI_F(uBias, SAMPLER, (int2)(oz, 0));
        int batchIndex = pos.y / dstChannelC4;
        ...
        batchIndex = batchOffset;
        {
            int oyStart   = realPos.y << 1;
            int oxStart   = realPos.x << 1;
            int pos_dst_x = dstX;
            FLOAT4 S00  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S10  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S20  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S30  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S01  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S11  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S21  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S31  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S02  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S12  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S22  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S32  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S03  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S13  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S23  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            pos_dst_x  += srcWidth;
            FLOAT4 S33  = RI_F(uInput, SAMPLER, (int2)(pos_dst_x, dstY));
            FLOAT4 m00  = +S00 + S01 + S02;
            FLOAT4 m10  = +S10 + S11 + S12;
            FLOAT4 m20  = +S20 + S21 + S22;
            FLOAT4 m30  = +S30 + S31 + S32;
