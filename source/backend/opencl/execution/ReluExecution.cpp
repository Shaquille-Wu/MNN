...
//Shaquille, Modified 20210125 Start
#if 0
	mUnits[0].kernel = runTime->buildKernel("binary", "binary", { "-DOPERATOR=select(in0*in1,in0,in0>=(FLOAT4)0)" });
#else
	std::set<std::string> build_options;
	if (imageWidth != globalSize[0] || imageHeight != globalSize[1])
	{
		build_options.emplace("-DOPERATOR=select(in0*in1,in0,in0>=(FLOAT4)0)");
		build_options.emplace("-DCHECK_IMAGE_BORDER");
	}
	mUnits[0].kernel = runTime->buildKernel("binary", "binary", build_options);
#endif
//Shaquille, Modified 20210125 End
