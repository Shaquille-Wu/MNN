...
#else
        const int strideDepth = (strides->Get(0) & 0x0000FFFF), strideHeight = strides->Get(1), strideWidth = strides->Get(2);
#endif
...
//Shaquille, Added 20210214 Start
        if (nullptr != context.get_first_backend() && MNN_FORWARD_OPENCL == context.get_first_backend()->type())
        {
            auto in_fmt       = TensorUtils::getDescribe(input)->dimensionFormat;
            auto out_fmt      = TensorUtils::getDescribe(output)->dimensionFormat;
            auto conv3d_param = op->main_as_Convolution3D();
            //auto pad_mode = common->padMode();
            if (MNN_DATA_FORMAT_NC4HW4 == in_fmt && MNN_DATA_FORMAT_NC4HW4 == out_fmt)
            {
                const int dialte_z = (dialteDepth & 0x0000FFFF);
                const int dialte_y = (strideHeight & 0x0000FFFF);
                const int dialte_x = (dialteWidth & 0x0000FFFF);
                if (3 == kernelDepth && 3 == kernelHeight && 3 == kernelWidth &&
                    1 == strideDepth && 1 == strideHeight && 1 == strideWidth &&
                    1 == dialte_z && 1 == dialte_y && 1 == dialte_x &&
                    1 == padDepth && 1 == padHeight && 1 == padWidth)
                {
                    Command cmd;
                    cmd.op      = op;
