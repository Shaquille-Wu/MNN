...
#else
        int32_t stride_y = layer->strideY();
        int32_t stride_x = layer->strideX();
        stride_y         = stride_y & 0x0000FFFF;
        if (layer->padMode() == PadMode_SAME) {
            // Tensorflow padding mode SAME
            output_width  = ceil((float)input->width() / (float)stride_x);
            output_height = ceil((float)input->height() / (float)stride_y);
        } else if (layer->padMode() == PadMode_VALID) {
            // Tensorflow padding mode VALID
            output_width  = ceil((float)(input->width() - kernel_width + 1) / (float)stride_x);
            output_height = ceil((float)(input->height() - kernel_height + 1) / (float)stride_y);
        } else {
            // Pad_Caffe means User setted padding
            if (nullptr != layer->pads()) {
                MNN_ASSERT(layer->pads()->size() >= 4);
