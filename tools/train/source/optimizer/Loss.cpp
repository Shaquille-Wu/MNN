...
Express::VARP _BCE_CrossEntropy(Express::VARP predicts, Express::VARP oneHotTargets) {
    MNN_ASSERT(predicts->getInfo()->dim.size() == 2);
    MNN_ASSERT(predicts->getInfo()->dim == oneHotTargets->getInfo()->dim);
    auto loss0 = _Negative(_ReduceMean(_ReduceSum(_Log(predicts) * oneHotTargets, {1}), {}));
    auto loss1 = _Negative(_ReduceMean(_ReduceSum(_Log(_Const(1.000001f) - predicts) * (_Const(1.0f) - oneHotTargets), {1}), {}));
    auto loss  = _Add(loss0, loss1);
    return loss;
}
